...
def call_gemini():
    ...
    system_prompt = "Reply like a pirate"
    content = text_box.get(1.0, tk.END)  # Get the user's input from the text box
    text_box.delete(1.0, tk.END)  # Clear the text box content

    client = genai.Client(api_key=gemini_key)  # Initialize the Gemini client
    response = client.models.generate_content(
        model='gemini-2.0-flash', contents=[system_prompt + content],  # Get the user's input from the text box
    )

    # Print the GPT response (can be used for debugging)
    print(response.text)
    # Open a new window and display the GPT response
    open_new_window(response.text)
...
submit_button = tk.Button(root, text="Submit", command=call_gemini)
